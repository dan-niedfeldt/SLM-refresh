...
def training_function(script_args, training_args):
    dataset = load_dataset("boyiwei/newsqa_filtered_sorted", split="train")
    dataset = dataset.shuffle(seed=23)
    train_dataset = dataset.select(range(1000))
    eval_dataset = dataset.select(range(1000, 1010))

    def create_conversation(sample):
        system_message = (
            "You are a financial news analyst assistant. Users will ask you questions in English and you will generate an "
            "accurate answer based on the provided NEWS_CONTEXT.\nNEWS_CONTEXT:\n{news_context}"
        )
        return {
            "messages": [
                {
                    "role": "system",
                    "content": system_message.format(news_context=sample["story_text"]),
                },
